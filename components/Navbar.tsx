...
const Navbar: React.FC<NavbarProps> = ({ setStickyNavVisible }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        enlargedQRCodeRef.current &&
        !enlargedQRCodeRef.current.contains(event.target as Node)
      ) {
        setIsEnlarged(false);
      }
    };
    ...
    <nav className={styles.navbar}>
      <div className={styles.left}>
        <div className={styles.logo}>
          <img src="/img/Dr.Hartls.png" alt="Härtl logo" />
      ...
      <div
        ref={navExtrasRef}
        className={`${styles.navExtras} ${styles.originalNavExtras}`}
